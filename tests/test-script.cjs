...
const source = fs.readFileSync(path.join(__dirname, '../src/Integration/ClashScript.cs'), 'utf8').split('private const string Template = """')[1].split('""";')[0].replaceAll('__SOCKS_PORT__', '1080').replaceAll('__VM_CIDR__','192.168.229.10/32').replaceAll('__VM_RULE_TYPE__','IP-CIDR');
...
for (const mode of ['blacklist', 'whitelist', 'rule']) {
  ...
  assert(result.proxies.some(p=>p.name==='Host2VMRelay'));
...
assert.equal(generated['rule-providers']['host2vm-relay-rules'].type,'file');
assert.equal(generated['rule-providers']['host2vm-relay-rules'].path,'./rules/host2vm-relay-rules.txt');
