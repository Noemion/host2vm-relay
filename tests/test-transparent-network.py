...
def setup():
    ...
    run('ip', '-n', CLIENT, 'route', 'add', 'blackhole', '198.19.0.3/32')
    run('ip', '-n', CLIENT, 'route', 'add', '198.19.0.2/32', 'via', '10.203.0.1')
...
def tests():
    ...
    record('original hostname resolves through fake-IP and VM DNS')
    ...
    record('VM network loss automatically falls back for TCP and UDP')
    ...
    expect_path('udp', 'HOST:probe'); expect_path('tcp', 'HOST')
    record('application crash with stale active rules still selects PASS and host path')
