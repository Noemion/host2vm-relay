...
运行 `Host2VMRelay-0.2.1-Setup.exe`。这是离线安装包，自动选择 x64、x86 或 ARM64 原生程序，安装到当前 Windows 用户的程序目录，提供开始菜单入口、可选桌面快捷方式及卸载入口。无需管理员权限，不会联网下载运行时，也不会在安装时编译。
...
配置保存在 `%LOCALAPPDATA%\Host2VMRelay`。程序只读取自身的配置目录，不自动导入其他程序或旧原型的设置。卸载仅删除程序文件，保留设置。跨用户迁移密码密文无效，需要重新输入。
