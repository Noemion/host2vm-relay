...
运行 `Host2VMRelay-0.2.0-Setup.exe`。这是离线安装包，自动选择 x64、x86 或 ARM64 原生程序，安装到当前 Windows 用户的程序目录，提供开始菜单入口、可选桌面快捷方式及卸载入口。无需管理员权限，不会联网下载运行时，也不会在安装时编译。
...
- 不支持 Windows 7/8/8.1、Windows Server Core 或非 Windows 客户端；远端 SSH 主机不限于麒麟或 Linux。
...
Clash Verge/Mihomo、虚拟机和 VPN 是外部网络环境，不随应用安装，不会替用户改动它们。要实现透明分流仍需运行 Clash TUN 并导入应用生成的脚本。
...
构建机需要 .NET 8 SDK 和 Inno Setup 7；首次 NuGet 还原需要网络。安装后的用户电脑不需要这些工具。

```powershell
.\build-release.ps1 -Iscc 'C:\Program Files\Inno Setup 7\ISCC.exe'
```
