...
public sealed partial class MainForm
{
    ...
    private static readonly string[] PageTitles = { "连接虚拟机", "转发规则", "Clash 接入", "运行日志" };
    private static readonly string[] PageDescriptions = {
        "通过 SSH，连接你信任的网络。", "只转发指定的域名、IP 与网段。",
        "一次接入，后续规则自动更新。", "连接、规则与诊断，一目了然。"
    };
    ...
    private void BuildShell()
    {
        ...
        compactNavigation = new Panel { Dock = DockStyle.Top, AutoSize = true, Margin = Padding.Empty, Padding = new Padding(16, 10, 16, 0), BackColor = UiTheme.Sidebar, Visible = false };
        shellBody = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 1, Margin = Padding.Empty };
        shellBody.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 204)); shellBody.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
        shellBody.RowStyles.Add(new RowStyle(SizeType.Percent, 100));
        sidebar = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 3, Margin = Padding.Empty, Padding = new Padding(14, 26, 14, 20), BackColor = UiTheme.Sidebar };
        ...
        var brand = UiLayout.Stack(); brand.Margin = new Padding(8, 0, 0, 26);
        brandImage = new PictureBox { Size = new Size(42, 42), SizeMode = PictureBoxSizeMode.Zoom, Margin = new Padding(0, 0, 0, 12), TabStop = false };
        using (var icon = AppIcon.Load(64)) brandImage.Image = icon.ToBitmap();
        UiLayout.Add(brand, brandImage); brandImage.Dock = DockStyle.None;
        UiLayout.Add(brand, UiLayout.Heading("Host2VMRelay", 12));
        UiLayout.Add(brand, UiLayout.Help("选择性网络中继"));
        ...
        string[] labels = { "连接", "转发规则", "Clash 接入", "运行日志" };
        for (int i = 0; i < labels.Length; i++)
        {
            var button = new NavigationButton { PageIndex = i, Text = labels[i], AccessibleName = PageTitles[i], Name = "navigation-" + i, Margin = new Padding(0, 0, 0, 8) };
            button.Click += (_, _) => SelectPage(button.PageIndex);
            navigationButtons.Add(button); navigation.Controls.Add(button);
        }
        sidebar.Controls.Add(navigation, 0, 1);
        var footer = UiLayout.Stack();
        UiLayout.Add(footer, UiLayout.Help("仅转发选定流量"));
        UiLayout.Add(footer, UiLayout.Help("v" + (typeof(MainForm).Assembly.GetName().Version?.ToString(3) ?? "")));
        sidebar.Controls.Add(footer, 0, 2);
        contentArea = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 3, Margin = Padding.Empty, Padding = new Padding(26, 22, 26, 12), BackColor = UiTheme.Canvas };
        ...
        var header = new TableLayoutPanel { Dock = DockStyle.Top, AutoSize = true, ColumnCount = 2, RowCount = 2, Margin = new Padding(0, 0, 0, 16) };
        ...
        pageDescription = UiLayout.Help(PageDescriptions[0]);
        state.Anchor = AnchorStyles.Top | AnchorStyles.Right; state.Margin = new Padding(12, 6, 0, 0);
        ...
        feed.Margin = new Padding(0, 10, 0, 0); contentArea.Controls.Add(feed, 0, 2); UiLayout.WrapLabels(contentArea);
        ...
        SizeChanged += (_, _) => UpdateShellLayout();
        FormClosed += (_, _) => { brandImage.Image?.Dispose(); brandImage.Image = null; };
        UpdateShellLayout();
    }
    ...
    private void SelectPage(int index)
    {
        ...
        tabs.SelectedIndex = index; RefreshNavigation();
    }
    ...
    private void UpdateShellLayout()
    {
        ...
        try
        {
            bool compact = ClientSize.Width * 96.0 / Math.Max(96, DeviceDpi) < 860;
            ...
            navigation.WrapContents = compact; navigation.AutoSize = compact;
            navigation.Dock = compact ? DockStyle.Top : DockStyle.Fill;
            string[] shortNames = { "连接", "规则", "Clash", "日志" };
            string[] longNames = { "连接", "转发规则", "Clash 接入", "运行日志" };
            foreach (var button in navigationButtons)
            {
                button.Compact = compact; button.Text = compact ? shortNames[button.PageIndex] : longNames[button.PageIndex];
                button.MinimumSize = new Size(UiTheme.Px(this, compact ? 82 : 174), UiTheme.Px(this, 44));
                button.Margin = new Padding(0, 0, UiTheme.Px(this, compact ? 6 : 0), UiTheme.Px(this, 6));
            }
            contentArea.Padding = new Padding(UiTheme.Px(this, compact ? 16 : 26), UiTheme.Px(this, compact ? 14 : 22), UiTheme.Px(this, compact ? 16 : 26), UiTheme.Px(this, 12));
        }
    ...
    {
        for (int i = 0; i < 4; i++)
            if (keyData == (Keys.Alt | (Keys)((int)Keys.D1 + i))) { SelectPage(i); return true; }
        if (keyData == (Keys.Control | Keys.Tab)) { SelectPage((tabs.SelectedIndex + 1) % 4); return true; }
        if (keyData == (Keys.Control | Keys.Shift | Keys.Tab)) { SelectPage((tabs.SelectedIndex + 3) % 4); return true; }
