...
public sealed partial class MainForm
{
    ...
    private static readonly string[] PageDescriptions = { "通过 SSH，连接你信任的网络。", "只转发指定的域名、IP 与网段。", "一次接入，后续规则自动更新。", "连接、规则与诊断，一目了然。" };
    ...
    private void BuildShell()
    {
        ...
        shellBody.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 204)); shellBody.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100)); shellBody.RowStyles.Add(new RowStyle(SizeType.Percent, 100));
    ...
    private void UpdateShellLayout()
    {
        ...
        try
        {
            ...
            string[] shortNames = { "连接", "规则", "Clash", "日志" }; string[] longNames = { "连接", "转发规则", "Clash 接入", "运行日志" };
    ...
    {
        for (int i = 0; i < 4; i++) if (keyData == (Keys.Alt | (Keys)((int)Keys.D1 + i))) { SelectPage(i); return true; }
