...
public sealed class MainForm : Form
{
    ...
    private readonly Label state = new() { AutoSize = true, Text = "● 未连接", ForeColor = Color.FromArgb(88, 104, 127) }, feed = new() { AutoSize = true, Text = "Clash 本地规则 · 未启用" };
    private readonly NotifyIcon tray = new() { Icon = SystemIcons.Application, Text = "Host2VMRelay", Visible = true };
    ...
    public MainForm()
    {
        ...
        heading.Controls.Add(new Label { Text = "Host2VMRelay", Font = new Font(Font.FontFamily, 28, FontStyle.Bold, GraphicsUnit.Pixel), AutoSize = true });
        ...
        FormClosing += (_, e) => { if (!quitting) { e.Cancel = true; Hide(); tray.ShowBalloonTip(2000, "Host2VMRelay", "已缩到托盘，后台连接继续运行。右键托盘图标可退出。", ToolTipIcon.Info); } };
        FormClosed += (_, _) => { wanted = false; timer.Stop(); Cleanup(); try { ClashRuleFile.Disable(); } catch { } tray.Dispose(); };
        timer.Tick += async (_, _) => {
            bool connected = client?.IsConnected == true;
            feed.Text = connected ? "Clash 本地规则 · 已启用" : "Clash 本地规则 · 未启用";
            if (wanted && !busy && !connected) {
                try { ClashRuleFile.Disable(); } catch (Exception ex) { Log("停用 Clash 本地规则失败：" + ex.Message); }
        ...
        timer.Start(); Log("已初始化 Clash 本地规则。先连接虚拟机，再到「接入 Clash」完成一次性配置。");
    }
    ...
    private void BuildRules()
    {
        ...
        var bar = new FlowLayoutPanel { Dock = DockStyle.Fill }; var save = new Button { Text = "保存规则", Width = 120, Height = 38 }; save.Click += (_, _) => { try { var payload = Rules.Compile(ruleText.Text); settings.Rules = ruleText.Text.Replace("\r", ""); settings.Save(); if (client?.IsConnected == true) ClashRuleFile.Write(payload); Log("规则已保存；Clash 本地规则会自动重新读取。"); MessageBox.Show("已保存。隧道已连接时规则会自动生效；DNS 旧缓存可能需刷新。", "规则已更新"); } catch (Exception ex) { Error(ex); } }; bar.Controls.Add(save); bar.Controls.Add(new Label { Text = "规则变更无需重复粘贴脚本；断开隧道后规则自动停用。", AutoSize = true, Margin = new Padding(12) }); g.Controls.Add(bar, 0, 2); p.Controls.Add(g);
    }
    private void BuildClash()
    {
        var p = Page("接入 Clash"); var g = Grid(p);
        Row(g, "① 复制脚本", new Label { AutoSize = true, MaximumSize = new Size(620, 0), Text = "在 Clash Verge Rev 当前订阅的「编辑扩展脚本」中替换为本应用生成的脚本，保存并应用订阅。脚本保留你已有的 mieru UDP 设置，并接入本地文件规则。其他自定义逻辑需手动合并。" });
        var copy = new Button { Text = "复制 Clash 扩展脚本", Height = 40 }; copy.Click += (_, _) => { try { ClashRuleFile.Disable(); Clipboard.SetText(ClashScript.Generate((int)socksPort.Value, host.Text.Trim())); } catch (Exception ex) { Error(ex); return; } Log("扩展脚本已复制到剪贴板。"); MessageBox.Show("已复制。请粘贴到当前订阅的扩展脚本并应用。", "接入 Clash"); }; Row(g, "", copy);
        ...
        Row(g, "③ 验证", new Label { AutoSize = true, MaximumSize = new Size(620, 0), Text = "连接隧道后，使用 Chrome 打开目标网页。在 Clash 连接列表确认命中「Host2VMRelay」。底部状态显示本地规则是否启用。\n若 Chrome 自定义了安全 DNS，请先改用系统 DNS。规则中也需包含登录跳转域名。" });
    ...
    private async Task Connect(bool automatic = false)
    {
        ...
        catch (Exception ex) { Cleanup(); try { ClashRuleFile.Disable(); } catch { } state.Text = "● 连接失败"; state.ForeColor = Color.Firebrick; Log("连接失败：" + ex.Message); if (!automatic) MessageBox.Show(ex.Message + "\n\n若 1080 端口被占用，请先关闭原来的 PowerShell 隧道。", "连接失败"); }
        finally {
    ...
    private void Stop() { if (busy) return; wanted = false; Cleanup(); try { ClashRuleFile.Disable(); } catch (Exception ex) { Log("停用 Clash 本地规则失败：" + ex.Message); } SetConnectionControls(false); state.Text = "● 已断开"; state.ForeColor = Color.DimGray; feed.Text = "Clash 本地规则 · 未启用"; Log("已断开隧道，Clash 本地规则已停用。"); }
}
