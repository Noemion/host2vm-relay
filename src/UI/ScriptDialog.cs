...
internal sealed class ScriptDialog : Form
{
    ...
    public ScriptDialog(Func<string?, string> generate, string existingScript)
    {
        ...
        source.TextChanged += (_, _) => InvalidateOutput(); import.Click += (_, _) => Import(); compose.Click += (_, _) => GenerateAndCopy();
    ...
    {
        if (layingOut || originalCard is null || generatedCard is null || chrome is null) return;
        layingOut = true;
        try
        {
            ...
            {
                editors.Controls.Clear(); editors.ColumnStyles.Clear(); editors.RowStyles.Clear(); editors.ColumnCount = columns; editors.RowCount = dual ? 1 : 2;
            ...
            originalFrame.MinimumSize = generatedFrame.MinimumSize = new Size(0, UiTheme.Px(this, 180)); originalFrame.Height = generatedFrame.Height = height;
    ...
    private void InvalidateOutput() { output.Clear(); copy.Enabled = save.Enabled = false; status.Text = "输入已变更，请重新生成。"; }
    private void Import()
    {
